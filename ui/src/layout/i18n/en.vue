<template></template>
<i18n>
{
  "en": {
    "search": "Search",
    "logOut": "LogOut",
    "close": "Close",
    "sureDelete": "Are you sure to Delete？",
    "sureRetry": "Are you sure to Retry？",
    "sureUnlock": "Are you sure to Unlock？",
    "success": "Success",
    "fail": "Fail",

    "noMsg": "Hurrah! We processed all messages.",

    "home": "Home",
    "schedule": "Schedule",
    "channel": "Channel",
    "log": "Log",
    "evengLog": "Event Log",
    "add": "Add",
    "edit": "Edit",
    "delete": "Delete",
    "retry": "Retry",
    "dlqLog": "DLQ Log",
    "workflowLog": "Workflow Log",
    "sequenceLock": "Sequence Lock",
    "redis": "Redis",
    "info": "Info",
    "command": "Command",
    "setting": "Setting",
    "operationLog": "Operation Log",
    "user": "User",
    "role": "Role",
    "config":"Config",
    "retryWarningHtml": "Warning: Item retry cannot be undone! Please proceed with caution!",
    "retryInfoHtml": "This operation will permanently retry the data of log.To prevent accidental actions, please confirm by entering the following:",
    "unlockInfoHtml": "This operation will permanently delete data. To prevent accidental operation, please enter the administrator password:",
    "deleteRoleWarningHtml": "Warning: This action cannot be undone! Please proceed with caution!",
    "deleteRoleInfoHtml": "This operation will permanently delete the data from the database. To avoid unintentional actions, please confirm by entering the following id:"
  }
}
</i18n>