...
3. When `normalQueuePartitions = 0` or `sequenceQueuePartitions = 0`, the value falls back to `defaultPartitions` for compatibility. Any final non-positive value is normalized to `1`.
...
The following functions can be used on `Client` and `BQClient` to select tenant connections and tune publish and retry behavior per message.

| Function | Scope | Description |
| --- | --- | --- |
| `WithTenant(string)` | Client | Switches the client to the Redis and MongoDB connections configured for the specified tenant in the default MongoDB `tenants` collection. |
...
Use `WithTenant` before calling `BQ` to publish or consume messages through a tenant's Redis and MongoDB connections. The tenant must exist in the default MongoDB `tenants` collection and contain valid connection settings.
...
| `normalQueuePartitions` | 0 | Fixed partitions for Normal Queue and Delay Queue (`0` falls back to `defaultPartitions`); cannot change after queue metadata is created |
| `sequenceQueuePartitions` | 0 | Fixed sequence queue scheduler partitions (`0` falls back to `defaultPartitions`); cannot change after queue metadata is created |
